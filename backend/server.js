...
app.post('/api/insertCertificate', async (req, res) => {
  const { imageLink, workerAddr, certificateId, receiptDate } = req.body
  ...
  try {
    ...
    for (let i = 0; i < certificates.length; i++) {
      ...
      let userInfo = await usersInfo.findOne({
        address: certificates[i].workerAddr.toLowerCase(),
      })
...
app.get('/api/getReviewsQueue', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < reviews.length; i++) {
      ...
      let userInfoFrom = await usersInfo.findOne({
        address: reviews[i].reviewFrom.toLowerCase(),
      })
      let userInfoTo = await usersInfo.findOne({
        address: reviews[i].reviewTo.toLowerCase(),
      })
    ...
    if (resp[i].data?.data.vc.credentialSubject?.data?.type) {
      if (resp[i].data?.data.vc.credentialSubject?.data?.type === 'MAIN') {
        ...
        responseFinalle.push({
          ...
          dateOfHire:
            resp[i].data?.data?.vc?.credentialSubject?.data?.dateOfHire,
      ...
      if (resp[i].data?.data.vc.credentialSubject?.data?.type === 'REVIEW') {
        ...
        responseFinalle.push({
          ...
          reviewText:
            resp[i].data?.data?.vc?.credentialSubject?.data?.reviewText,
          reviewType:
            resp[i].data?.data?.vc?.credentialSubject?.data?.reviewType,
